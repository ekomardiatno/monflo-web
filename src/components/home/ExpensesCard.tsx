...
export default function ExpensesCard({
  ...
  const amountVisibility = useAppSelector(
    (state) => state.app.amountVisibility,
  );
  ...
  const currentMonthActivities = useMemo(() => {
    const filtered = activities.filter((activity) => {
      ...
      return (
        d.getTime() >= startDateCurrentMonth.getTime() &&
        d.getTime() <= endDateCurrentMonth.getTime() &&
        activity.expense
      );
    });
    ...
    activities,
    startDateCurrentMonth.getTime(),
    endDateCurrentMonth.getTime(),
  ]);
  ...
  const lastMonthActivities = useMemo(() => {
    const filtered = activities.filter((activity) => {
      const d = new Date(activity.date);
      return (
        d.getTime() >= startDateLastMonth.getTime() &&
        d.getTime() <= endDateLastMonth.getTime() &&
        activity.expense
      );
    });
    return filtered.reduce((acc: Record<string, number>, item) => {
  ...
  const cardShadow =
    theme.schema === "DARK" ? CARD_SHADOW_DARK : CARD_SHADOW_LIGHT;

  return (
    <div>
      <p
        className="text-[13px] font-semibold px-5 mb-2.5 lg:px-0"
        style={{ color: theme.textHintColor }}
      >
        {capitalizeFirstText(monthDifferenceTitle(startDateCurrentMonth))}{" "}
        expenses
      </p>
      <div
        ref={scrollRef}
        ...
        <div className="pl-5 lg:pl-0">
          <div className="flex gap-2.5 pb-1">
            {categoriesAmount.map((row) => {
              ...
              const trendColor =
                diff > 0
                  ? COLORS.colorDanger500
                  : diff < 0
                    ? COLORS.colorSuccess500
                    : theme.textHintColor;
              return (
                <button
                  key={row.category}
                  ...
                  onClick={() => {
                    ...
                    navigate(
                      `/allocation/${dayjs(startDateCurrentMonth).format("YYYY-MM-DD")}/expense/${row.category}`,
                    );
                  }}
                >
                  ...
                  <p
                    className="text-[17px] font-bold font-mono mb-2.5"
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    >
                      {!amountVisibility
                        ? HIDDEN_AMOUNT_TEXT
                        : `${diff > 0 ? "+" : ""}${numeral(diff).format("0,0")}`}
                    </p>
                  </div>
                </button>
              );
            })}
            <div className="shrink-0 w-2.5 lg:w-0" />
          </div>
        </div>
      </div>
    </div>
